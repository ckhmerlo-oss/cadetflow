...
async function convertToDemerit(incidentId, offenseTypeId, notes) {
    ...
    const { data: newReport } = await supabase.from('demerit_reports').select('id').eq('submitted_by', user.id) // It was just created by me
    .eq('subject_cadet_id', incident.subject_cadet_id).order('created_at', {
        ascending: false
    }).limit(1).single();
    if (newReport) {
...
"[project]/.next-internal/server/app/incidents/create/page/actions.js { ACTIONS_MODULE0 => \"[project]/app/incidents/actions.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
"[project]/.next-internal/server/app/incidents/create/page/actions.js { ACTIONS_MODULE0 => \"[project]/app/incidents/actions.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__) => {
"use strict";

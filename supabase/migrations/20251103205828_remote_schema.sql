alter table "public"."demerit_reports" drop constraint "check_content_structure";

drop function if exists "public"."create_new_report"(p_title text, p_subject_cadet_id uuid, p_content jsonb, p_date_of_offense date);

alter table "public"."demerit_reports" drop column "content";

alter table "public"."demerit_reports" drop column "title";

alter table "public"."demerit_reports" add column "notes" text;

alter table "public"."demerit_reports" add column "offense_type_id" uuid not null;

alter table "public"."demerit_reports" add constraint "demerit_reports_offense_type_id_fkey" FOREIGN KEY (offense_type_id) REFERENCES public.offense_types(id) not valid;

alter table "public"."demerit_reports" validate constraint "demerit_reports_offense_type_id_fkey";
...
DECLARE
    ...
    v_term RECORD;
    v_report_data RECORD; -- Renamed from v_report
...
BEGIN
    ...
    LOOP
        ...
        -- *** REFACTORED: JOIN with offense_types ***
...
BEGIN
  ...
  -- 3. Create the new report (Refactored INSERT)
...
BEGIN
    ...
    IF my_level >= 50 THEN
        ...
        SELECT 
            dr.id,
            ot.offense_name AS title, -- *** REFACTORED ***
            dr.status,
            dr.created_at,
        ...
        LEFT JOIN public.offense_types AS ot ON dr.offense_type_id = ot.id -- *** REFACTORED ***
...
BEGIN
  ...
  IF current_group_chain.next_approver_group_id IS NULL THEN
    ...
    -- *** REFACTORED: Get demerit count from offense_types table ***
    SELECT ot.demerits
...
AS $function$
BEGIN
  IF NOT public.is_member_of_approver_group(
    (SELECT current_approver_group_id FROM public.demerit_reports WHERE id = p_report_id)
  ) THEN
    RAISE EXCEPTION 'You do not have permission to act on this report.';
  END IF;
  UPDATE public.demerit_reports

-- Mark barracks rooms as supply, CQ, or other non-cadet purposes from the room page.

set check_function_bodies = off;

alter table public.barracks_rooms
  add column if not exists room_purpose text;

alter table public.barracks_rooms
  drop constraint if exists barracks_rooms_room_purpose_check;

alter table public.barracks_rooms
  add constraint barracks_rooms_room_purpose_check
  check (room_purpose is null or room_purpose in ('supply', 'cq', 'special'));

comment on column public.barracks_rooms.room_purpose is
  'Non-cadet room designation. Null = standard cadet room. Clears occupants when set.';

create or replace function public.set_barracks_room_purpose(
  p_room_id uuid,
  p_purpose text
)
returns void
...
begin
  ...
  if v_purpose is not null and v_purpose not in ('supply', 'cq', 'special') then
    raise exception '[set_barracks_room_purpose] Invalid room purpose: %', v_purpose;
  end if;
...
begin
  ...
  if v_room.room_purpose is not null then
    raise exception '[assign_barracks_bunk] Room is marked as % and cannot accept cadets', v_room.room_purpose;
  end if;
...
grant execute on function public.set_barracks_room_purpose(uuid, text) to authenticated;

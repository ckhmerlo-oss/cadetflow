...
CREATE OR REPLACE FUNCTION public.calculate_tours_for_new_report(p_cadet_id uuid, p_demerits integer, p_policy_category integer, p_date_of_offense date)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE p_date_of_offense BETWEEN start_date AND end_date
  LIMIT 1;
  ...
  IF v_first_cat3_date IS NOT NULL AND p_date_of_offense >= v_first_cat3_date THEN
  ...
  IF p_policy_category != 3 AND v_credits_nuked = false THEN
    ...
    WHERE tl.cadet_id = p_cadet_id
      ...
      AND r.status = 'completed'
      -- CRITICAL: ExCpldue the current report being calculated to avoid infinite recursion or double counting if called during an update
      AND r.date_of_offense <= p_date_of_offense 
      AND r.created_at < now(); -- rough check to ensure we only look at *past* reports

...
import { setupSchoolYearTerms } from '@/app/oversight/actions'
...
export default function GeneralSettingsTab() {
  ...
  const [yearTerms, setYearTerms] = useState(() => defaultTermNames('2025-2026').map((name, i) => {
    const base = new Date()
    const start = new Date(base)
    start.setDate(start.getDate() + i * 30)
    const end = new Date(start)
    end.setDate(end.getDate() + 29)
    return { name, start: start.toISOString().slice(0, 10), end: end.toISOString().slice(0, 10) }
  }))
  ...
  useEffect(() => {
    ...
    setYearTerms((prev) => prev.map((t, i) => ({ ...t, name: names[i] ?? t.name })))
  }, [schoolYear])

  async function fetchAdminData() {
    setIsLoadingData(true)
    const [termsRes, activeYearRes] = await Promise.all([
      supabase.rpc('get_all_academic_terms'),
      supabase.rpc('get_active_school_year'),
    ])
    if (termsRes.error) setDataError(termsRes.error.message)
    else setTerms(termsRes.data as AcademicTerm[])

    const activeYear = (activeYearRes.data as string | null) ?? null
    setActiveSchoolYear(activeYear)
    if (activeYear) {
      setSchoolYear((prev) => prev || activeYear)
      setYearTerms((prev) => {
        const names = defaultTermNames(activeYear)
        return prev.map((t, i) => ({ ...t, name: names[i] ?? t.name }))
      })
    }
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      {activeSchoolYear && (
        <div className="p-4 bg-primary/10 border border-primary/30 rounded-lg">
          <p className="text-sm font-semibold text-foreground">
            Active school year: <span className="text-primary">{activeSchoolYear}</span>
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            Operational year for rosters, classes, and reporting. Post-close, this may be the next configured year before its first term starts.
          </p>
        </div>
      )}
      ...
      <AdminSection title="School Year Setup (5 Terms)">
        ...
        {activeYears.length > 0 && (
          <div className="mt-4">
            <a
              href="/admin/year-close"
              className="inline-flex text-sm px-4 py-2 bg-destructive/10 border border-destructive/30 text-destructive rounded hover:bg-destructive/20"
            >
              Close school year (wizard)
            </a>
            <p className="text-xs text-muted-foreground mt-2">
              Use the Close School Year wizard for end-of-year archive. Individual term setup remains above.
            </p>
          </div>
        )}
        ...
        <div className="mt-4 overflow-x-auto border border-border rounded-lg">
            <table className="min-w-full divide-y divide-border">
                ...
                <tbody className="bg-card divide-y divide-border">
                    {terms.map(t => (
                        <tr key={t.id} className={t.school_year === activeSchoolYear && !t.archived ? 'bg-primary/5' : undefined}>
                            <td className="px-6 py-4 text-sm text-muted-foreground">
                              {t.school_year}
                              {t.school_year === activeSchoolYear && !t.archived && (

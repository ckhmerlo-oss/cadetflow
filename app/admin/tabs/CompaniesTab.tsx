...
export default function CompaniesTab() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (result?.error) alert(result.error); else { setNewName(''); fetchCompanies(); }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result?.error) alert(result.error); else fetchCompanies()
  }

  if (loading) return <div className="p-8 text-muted-foreground">Loading companies...</div>

  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-foreground">Company Units</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-card shadow-sm rounded-lg border border-border overflow-hidden">
            <ul className="divide-y divide-border">
                {companies.length === 0 ? (
                    <li className="p-4 text-center text-muted-foreground">No companies found.</li>
                ) : (
                    companies.map(c => (
                        <li key={c.id} className="p-4 flex justify-between items-center hover:bg-muted/50 transition-colors">
                            <span className="font-medium text-foreground">{c.company_name}</span>
                            <button onClick={() => handleDelete(c.id)} className="text-muted-foreground hover:text-destructive transition-colors" title="Delete Company">
                                <TrashIcon />
                            </button>
                        </li>
                    ))
        ...
        {/* CREATE FORM */}
        <div className="bg-primary/5 p-6 rounded-lg border border-primary/20 h-fit">
            <h3 className="text-lg font-semibold text-primary mb-4">Add New Unit</h3>

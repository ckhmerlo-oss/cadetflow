...
export default function HeaderMenu({ isLoggedIn, canManage, showDailyReports, showClasses, isSiteAdmin, roleLevel, isInBand, isMaintenanceManager, isMaintenanceOnlyNav = false }: HeaderMenuProps) {
  ...
  const userMenuBlock = isLoggedIn ? (
    ...
    return (
      <>
        <div className="hidden md:flex items-center justify-end space-x-3">
          <Link href="/work-orders" id="nav-work-orders" className={navLinkClass}>
            Work Orders
          </Link>
          <Link href="/barracks/hallway" id="nav-hallway" className={navLinkClass}>
            Hallway
          </Link>
          <Link href="/submit?tab=damage" id="nav-new-request" className={navLinkClass}>
            + New Request
          </Link>
          {userMenuBlock}
        </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-16 inset-x-0 z-50 bg-popover border-b border-border shadow-lg">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              <Link href="/work-orders" className="block px-3 py-2 rounded-md text-base font-medium text-foreground hover:bg-muted/50 hover:text-primary">Work Orders</Link>
              <Link href="/barracks/hallway" className="block px-3 py-2 rounded-md text-base font-medium text-foreground hover:bg-muted/50 hover:text-primary">Hallway</Link>
              <Link href="/submit?tab=damage" className="block px-3 py-2 rounded-md text-base font-medium text-foreground hover:bg-muted/50 hover:text-primary">+ New Request</Link>
            </div>
            {isLoggedIn && (
  ...
  return (
    <>
      ...
      <div className="hidden md:flex items-center justify-end space-x-3">
        ...
        {roleLevel >= 15 && roleLevel < 65 && (
             <Link href="/work-orders" id="nav-my-requests" className={navLinkClass}>
                My Requests
            </Link>
        )}

        {roleLevel >= 65 && !isMaintenanceManager && (
             <Link href="/barracks/hallway" id="nav-hallway" className={navLinkClass}>
                Hallway
            </Link>
        )}
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-16 inset-x-0 z-50 bg-popover border-b border-border shadow-lg">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {roleLevel >= 15 && roleLevel < 65 && (
                <Link href="/work-orders" className="block px-3 py-2 rounded-md text-base font-medium text-foreground hover:bg-muted/50 hover:text-primary">
                    My Requests
                </Link>
            )}

            {roleLevel >= 65 && !isMaintenanceManager && (
                <Link href="/barracks/hallway" className="block px-3 py-2 rounded-md text-base font-medium text-foreground hover:bg-muted/50 hover:text-primary">
                    Hallway
                </Link>
            )}

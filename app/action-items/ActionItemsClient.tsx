...
export default function ActionItemsClient({ initialReports, currentUserId }: { initialReports: ActionItemReport[], currentUserId: string }) {
  ...
  const processedReports = useMemo(() => {
    ...
    if (filterCategory === 'date_range') {
        ...
        switch (filterCategory) {
            ...
            case 'type': result = result.filter(r => getTaskType(r) === filterValue); break;
        }
    }
  ...
  const performAction = async (report: ActionItemReport, action: 'approve' | 'reject' | 'kickback', comment: string) => {
      ...
      if (taskType === 'Approval Needed') {
          ...
          if (rpcName) payload = { p_appeal_id: report.appeal_id, p_action: appealAction, p_comment: comment };
          else return { error: { message: 'Could not determine appeal stage.' } };
      }
  ...
  const SortIcon = ({ active, direction }: { active: boolean, direction: 'asc' | 'desc' }) => {
      if (!active) return <span className="ml-1 text-muted-foreground/50">↕</span>;
  ...
  const getTaskBadge = (r: ActionItemReport) => {
      ...
      const styles: Record<string, string> = {
          ...
          'Incident Review': 'bg-destructive/10 text-destructive border border-destructive/20', // Semantic Destructive
          'Revision Needed': 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-200',
          'Appeal Review': 'bg-primary/10 text-primary border border-primary/20', // Semantic Primary
          'Appeal Decision': 'bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-200',
      };
      return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${styles[type] || 'bg-muted text-muted-foreground'}`}>{type}</span>
  }
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="w-full lg:w-1/3 relative">
                ...
                <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg className="h-5 w-5 text-muted-foreground" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clipRule="evenodd" /></svg>
                    </div>
                    <input 
                        type="text" 
                     ...
                     {filterCategory === 'date_range' ? (
                        <div className="flex gap-2">
                            <div className="w-1/2">
                                ...
                                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="block w-full rounded-md border-input bg-background text-foreground shadow-sm sm:text-sm py-2 px-3" />
                            </div>
                            <div className="w-1/2">
                                <label className="block text-xs font-bold text-muted-foreground uppercase mb-1">To</label>
                                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="block w-full rounded-md border-input bg-background text-foreground shadow-sm sm:text-sm py-2 px-3" />
                            </div>
                        </div>
                     ) : (
                        <div>
                            <label className={`block text-xs font-bold uppercase mb-1 ${filterCategory === 'all' ? 'text-muted-foreground/50' : 'text-muted-foreground'}`}>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 p-3 bg-muted/30 rounded-md border border-border">
          ...
          <div className="flex-1 w-full">
             <input type="text" placeholder={selectedReports.size > 0 ? "Comment for bulk action (required for rejection)..." : "Select checkboxes to enable bulk actions..."} value={bulkComment} onChange={e => setBulkComment(e.target.value)} disabled={selectedReports.size === 0} className="block w-full rounded-md border-input bg-background text-foreground shadow-sm sm:text-sm py-2 px-3 disabled:bg-muted disabled:text-muted-foreground" />
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
             {/* Note: Kept specific colors for Accept/Reject as they are functional state colors */}
             <button onClick={() => handleBulkAction('approve')} disabled={selectedReports.size === 0 || isLoading} className="flex-1 sm:flex-none px-4 py-2 bg-green-600 text-white text-sm font-bold rounded-md hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-colors">Approve</button>
             <button onClick={() => handleBulkAction('reject')} disabled={selectedReports.size === 0 || isLoading || !bulkComment.trim()} className="flex-1 sm:flex-none px-4 py-2 bg-destructive text-destructive-foreground text-sm font-bold rounded-md hover:bg-destructive/90 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-colors">Reject</button>
          </div>
      </div>
        ...
        <div className="overflow-x-auto">
            ...
            <thead className="bg-muted/50">
                <tr>
                    ...
                    <th onClick={() => handleSort('created_at')} className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase cursor-pointer hover:bg-muted/80 transition-colors">Date <SortIcon active={sortConfig.key === 'created_at'} direction={sortConfig.direction} /></th>
                    <th onClick={() => handleSort('subject')} className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase cursor-pointer hover:bg-muted/80 transition-colors">Subject <SortIcon active={sortConfig.key === 'subject'} direction={sortConfig.direction} /></th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Infraction</th>
                    <th onClick={() => handleSort('type')} className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase cursor-pointer hover:bg-muted/80 transition-colors">Action <SortIcon active={sortConfig.key === 'type'} direction={sortConfig.direction} /></th>
                </tr>
            </thead>
            <tbody className="bg-card divide-y divide-border">
                {processedReports.length > 0 ? processedReports.map(item => {
                    ...
                    <React.Fragment key={item.id}>
                        <tr onClick={() => handleRowClick(item.id)} className={`cursor-pointer hover:bg-muted/50 transition-colors ${expandedRowId === item.id ? 'bg-muted/30' : ''}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{new Date(item.created_at).toLocaleDateString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm font-medium text-foreground">{formatName(item.subject)}</div><div className="text-xs text-muted-foreground">By: {formatName(item.submitter)}</div></td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">{item.offense_type.offense_name}</td>
                            ...
                            <div className="flex flex-col md:flex-row">
                                ...
                                <div className="flex-grow p-6 space-y-4 md:border-r border-border">
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div><span className="block text-xs font-bold text-muted-foreground uppercase">Submitted By</span><span className="text-foreground">{formatName(item.submitter)}</span></div>
                                        <div><span className="block text-xs font-bold text-muted-foreground uppercase">Time</span><span className="text-foreground">{new Date(item.created_at).toLocaleTimeString()}</span></div>
                                    </div>
                                    
                                    <div>
                                        <h4 className="text-xs font-bold text-muted-foreground uppercase mb-1">Original Report Notes</h4>
                                        <p className="text-sm text-foreground bg-card p-3 rounded border border-border whitespace-pre-wrap">
                                            {item.notes || <span className="italic text-muted-foreground">No notes provided.</span>}
                                        </p>
                                    </div>
                                    ...
                                    {isAppeal ? (
                                        <div className="space-y-3 mt-4">
                                            <h4 className="text-sm font-bold text-primary pb-1 border-b border-border">Appeal Case File</h4>
                                            
                                            <div className="bg-primary/5 p-3 rounded-md border border-primary/10">
                                                <span className="text-xs font-bold text-primary uppercase block mb-1">Cadet Justification</span>
                                                <p className="text-sm text-foreground">{item.appeal_justification}</p>
                                            </div>

                                            {item.appeal_issuer_comment && (
                                                <div className="ml-4 bg-muted p-3 rounded-md border-l-4 border-muted-foreground">
                                                    <span className="text-xs font-bold text-muted-foreground uppercase block mb-1">Issuer Rebuttal</span>
                                                    <p className="text-sm text-foreground">{item.appeal_issuer_comment}</p>
                                                </div>
                                            )}

                                            {item.appeal_chain_comment && (
                                                <div className="ml-8 bg-muted p-3 rounded-md border-l-4 border-muted-foreground">
                                                    <span className="text-xs font-bold text-muted-foreground uppercase block mb-1">Chain of Command Note</span>
                                                    <p className="text-sm text-foreground">{item.appeal_chain_comment}</p>
                                                </div>
                                            )}
                                        </div>
                                    ) : (
                                            ...
                                            <div className="space-y-2 max-h-40 overflow-y-auto">
                                                {item.logs.map((log, idx) => (
                                                    <div key={idx} className="flex items-start gap-2 text-xs">
                                                        <span className="font-medium text-foreground w-24 flex-shrink-0">{log.actor_name}</span>
                                                        <span className="px-1.5 py-0.5 bg-muted rounded text-muted-foreground font-bold uppercase text-[10px]">{log.action}</span>
                                ...
                                <div className="md:w-72 flex-shrink-0 p-6 bg-muted/10 flex flex-col gap-4 border-l border-border">
                                        ...
                                        <>
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <button onClick={() => handleSingleAction(item, 'reject')} disabled={isLoading || !singleComment.trim()} className={`flex-1 py-2 ${isAppeal ? 'w-full' : ''} bg-destructive hover:bg-destructive/90 text-destructive-foreground rounded text-sm font-medium disabled:opacity-50 transition-colors shadow-sm`}>
                                                        {isAppeal ? 'Reject Appeal' : 'Reject'}
                                                    </button>
                                                </div>
                                            </div>
                                            <Link href={`/report/${item.id}`} className="text-center text-xs text-primary hover:underline mt-2 font-medium">
                    ...
                    <tr><td colSpan={5} className="px-6 py-12 text-center text-muted-foreground">No pending items found.</td></tr>
                )}
            </tbody>
            </table>
        </div>
